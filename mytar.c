...
void processFile(char* fileName, char **fileList, char **fileList2, bool t_Option, bool x_Option, bool v_Option, int filesCount){
    ...
    while (!end){
        ...
        if(fileName[0] == '\0'){
            ...
            filesize = (filesize % BUFFERSIZE == 0) ? filesize : (filesize + (BUFFERSIZE - (filesize % BUFFERSIZE)));
...
void handleOptions(char **fileName, int argc, char **argv, bool *t_Option, bool *f_Option, bool *v_Option, bool *x_Option){
...
void handleFileList(char **fileName, int argc, char **argv,char **fileList, char **fileList2, int *filesCount, bool getFileList){
...
int main(int argc, char *argv[]){
    ...
    handleOptions(&fileName,argc,argv,&t_Option,&f_Option,&v_Option,&x_Option);
    bool getFileList = (t_Option || v_Option);
    handleFileList(&fileName,argc,argv,fileList,fileList2,&filesCount,getFileList);
